...
ig.baked=true;ig.module('impact.entity').requires('impact.animation','impact.impact').defines(function(){"use strict";ig.Entity=ig.Class.extend({id:0,settings:{},size:{x:16,y:16},offset:{x:0,y:0},pos:{x:0,y:0},last:{x:0,y:0},vel:{x:0,y:0},accel:{x:0,y:0},angleVel:0,angleAcc:0,friction:{x:0,y:0},angleFriction:0,maxVel:{x:100,y:100},maxAngleVel:100,zIndex:0,gravityFactor:1,standing:false,bounciness:0,minBounceVelocity:40,anims:{},animSheet:null,currentAnim:null,health:10,type:0,checkAgainst:0,collides:0,_killed:false,slopeStanding:{min:(44).toRad(),max:(136).toRad()},init:function(x,y,settings){this.id=++ig.Entity._lastId;this.pos.x=this.last.x=x;this.pos.y=this.last.y=y;ig.merge(this,settings);},reset:function(x,y,settings){var proto=this.constructor.prototype;this.pos.x=x;this.pos.y=y;this.last.x=x;this.last.y=y;this.vel.x=proto.vel.x;this.vel.y=proto.vel.y;this.accel.x=proto.accel.x;this.accel.y=proto.accel.y;this.angleVel=proto.angleVel;this.angleAcc=proto.angleAcc;this.health=proto.health;this._killed=proto._killed;this.standing=proto.standing;this.type=proto.type;this.checkAgainst=proto.checkAgainst;this.collides=proto.collides;ig.merge(this,settings);},addAnim:function(name,frameTime,sequence,stop){if(!this.animSheet){throw('No animSheet to add the animation '+name+' to.');}
var a=new ig.Animation(this.animSheet,frameTime,sequence,stop);this.anims[name]=a;if(!this.currentAnim){this.currentAnim=a;}
return a;},update:function(){this.last.x=this.pos.x;this.last.y=this.pos.y;this.vel.y+=ig.game.gravity*ig.system.tick*this.gravityFactor;this.vel.x=this.getNewVelocity(this.vel.x,this.accel.x,this.friction.x,this.maxVel.x);this.vel.y=this.getNewVelocity(this.vel.y,this.accel.y,this.friction.y,this.maxVel.y);this.angleVel=this.getNewAngleVelocity(this.angleVel,this.angleAcc,this.angleFriction,this.maxAngleVel);var mx=this.vel.x*ig.system.tick;var my=this.vel.y*ig.system.tick;var res=ig.game.collisionMap.trace(this.pos.x,this.pos.y,mx,my,this.size.x,this.size.y);this.handleMovementTrace(res);this.currentAnim.angle+=this.angleVel*ig.system.tick;if(this.currentAnim){this.currentAnim.update();}},getNewVelocity:function(vel,accel,friction,max){if(accel){return(vel+accel*ig.system.tick).limit(-max,max);}
else if(friction){var delta=friction*ig.system.tick;if(vel-delta>0){return vel-delta;}
else if(vel+delta<0){return vel+delta;}
else{return 0;}}
return vel.limit(-max,max);},getNewAngleVelocity:function(vel,accel,friction,max){if(accel){return(vel+accel*ig.system.tick).limit(-max,max);}
else if(friction){var delta=friction*ig.system.tick;if(vel-delta>0){return vel-delta;}
else if(vel+delta<0){return vel+delta;}
else{return 0;}}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.entity-pool').defines(function(){ScreenWrapEntity=ig.Entity.extend({screenWrap:function(){if(this.pos.x-this.offset.x>ig.system.width){this.pos.x=-this.size.x-this.offset.x;}
else if(this.pos.x+this.size.x+this.offset.x<0){this.pos.x=ig.system.width+this.offset.x;}
if(this.pos.y-this.offset.y>ig.system.height){this.pos.y=-this.size.y-this.offset.y;}
else if(this.pos.y+this.size.y+this.offset.y<0){this.pos.y=ig.system.height+this.offset.y;}}});Ship=ScreenWrapEntity.extend({size:{x:50,y:50},checkAgainst:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/ship.png',50,60),maxVel:{x:300,y:300},maxAngleVel:5,rotationalAcc:3,forwardAcc:200,shotTimer:new ig.Timer(0.33),shotSpeed:300,health:3,invincibilityTimer:new ig.Timer(3),init:function(x,y,settings){this.parent(x,y,settings);this.pos.x-=this.size.x/2;this.pos.y-=this.size.y/2;this.addAnim('idle',0.1,[0]);this.addAnim('turn',0.1,[1,2,3]);this.addAnim('forward',0.1,[4,5,6]);this.shotTimer.reset();},update:function(){this.checkMovement();this.screenWrap();this.checkShooting();this.invincibilityBlink();this.parent();},checkMovement:function(){if(ig.input.state('left')&&!ig.input.state('right')){this.angleAcc=-this.rotationalAcc*((this.angleVel>0)?2:1);this.changeAnim(this.anims.turn);this.currentAnim.flip.x=true;}
else if(ig.input.state('right')&&!ig.input.state('left')){this.angleAcc=this.rotationalAcc*((this.angleVel<0)?2:1);this.changeAnim(this.anims.turn);this.currentAnim.flip.x=false;}
else{this.changeAnim(this.anims.idle);this.angleAcc=0;}
if(ig.input.state('up')){this.accel.x=this.xComponent(this.forwardAcc);this.accel.y=this.yComponent(this.forwardAcc);this.changeAnim(this.anims.forward);}
else{this.accel.x=this.accel.y=0;}},checkShooting:function(){if(ig.input.state('shoot')&&this.shotTimer.delta()>=0){centerX=this.pos.x+this.size.x/2+this.xComponent(this.size.x/2);centerY=this.pos.y+this.size.y/2+this.yComponent(this.size.y/2);velX=this.vel.x+this.xComponent(this.shotSpeed);velY=this.vel.y+this.yComponent(this.shotSpeed);ig.game.spawnEntity(PlayerBullet,centerX,centerY,{vel:{x:velX,y:velY}});this.shotTimer.reset();}},invincibilityBlink:function(){if(this.invincibilityTimer.delta()<0){this.currentAnim.alpha=(Math.random()>1/Math.abs(this.invincibilityTimer.delta()))?0:1;}
else this.currentAnim.alpha=1;},destroyShip:function(){this.pos.x=ig.system.width/2-this.size.x/2;this.pos.y=ig.system.height/2-this.size.y/2;this.vel={x:0,y:0};this.angleVel=0;this.changeAnim(this.anims.idle);this.currentAnim.angle=0;this.receiveDamage(1);this.invincibilityTimer.reset();if(this.health==0)ig.game.endGame();},xComponent:function(value){return Math.cos(this.currentAnim.angle-(Math.PI/2))*value;},yComponent:function(value){return Math.sin(this.currentAnim.angle-(Math.PI/2))*value;},check:function(other){if(this.invincibilityTimer.delta()<0)return;if(other instanceof Asteroid)
other.breakAsteroid(false);else if(other instanceof EnemyShip)
other.destroyShip(false);else if(other instanceof EnemyBullet)
other.kill();this.destroyShip();}});EnemyShip=ig.Entity.extend({size:{x:50,y:35},animSheet:new ig.AnimationSheet('media/enemy_ship.png',50,35),type:ig.Entity.TYPE.B,shipState:{state:"SEEKING",SEEKING:"SEEKING",MOVING:"MOVING",SHOOTING:"SHOOTING"},stateTimer:new ig.Timer(1),targetPos:{x:0,y:0},velocity:3,shotSpeed:200,pointValue:50,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0],true);},update:function(){if(this.stateTimer.delta()<0)return;switch(this.shipState.state){case this.shipState.SEEKING:this.seekShip();break;case this.shipState.MOVING:this.move();break;case this.shipState.SHOOTING:this.shoot();break;}},seekShip:function(){this.targetPos.x=Math.random()*(ig.system.width-this.size.x);this.targetPos.y=Math.random()*(ig.system.height-this.size.y);this.changeState(this.shipState.MOVING);},move:function(){distX=this.targetPos.x-this.pos.x;distY=this.targetPos.y-this.pos.y;distance=Math.sqrt(Math.pow(distX,2)+Math.pow(distY,2));if(distance<this.velocity){this.pos.x=this.targetPos.x;this.pos.y=this.targetPos.y;this.changeState(this.shipState.SHOOTING);return;}
angleTo=Math.atan2(distY,distX);velX=Math.cos(angleTo)*this.velocity;velY=Math.sin(angleTo)*this.velocity;this.pos.x+=velX;this.pos.y+=velY;},shoot:function(){angleTo=this.angleTo(ig.game.ship);velX=Math.cos(angleTo)*this.shotSpeed;velY=Math.sin(angleTo)*this.shotSpeed;newBullet=ig.game.spawnEntity(EnemyBullet,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{vel:{x:velX,y:velY}});newBullet.currentAnim.angle=angleTo;this.changeState(this.shipState.SEEKING);},changeState:function(state){this.shipState.state=state;this.stateTimer.reset();},destroyShip:function(score){if(score)
ig.game.score+=this.pointValue;ig.game.enemyTimer.reset();this.kill();}});Bullet=ScreenWrapEntity.extend({maxVel:{x:500,y:500},health:75,init:function(x,y,settings){console.log("bullet");console.log("bang");this.parent(x,y,settings);this.pos.x-=this.size.x/2;this.pos.y-=this.size.y/2;this.addAnim('idle',0.1,[0]);},reset:function(x,y,settings){this.parent(x,y,settings);this.pos.x-=this.size.x/2;this.pos.y-=this.size.y/2;},update:function(){this.screenWrap();this.receiveDamage(1);this.fadeBullet();this.parent();},fadeBullet:function(){if(this.health<10){this.currentAnim.alpha=this.health/10;}
else
this.currentAnim.alpha=1;}});PlayerBullet=Bullet.extend({size:{x:10,y:10},checkAgainst:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/bullet.png',10,10),check:function(other){if(other instanceof Asteroid)
other.breakAsteroid(true);else if(other instanceof EnemyShip)
other.destroyShip(true);else if(other instanceof EnemyBullet)
other.kill();this.kill();}});EnemyBullet=Bullet.extend({size:{x:10,y:5},type:ig.Entity.TYPE.B,health:150,animSheet:new ig.AnimationSheet('media/enemy_bullet.png',10,5)});Asteroid=ScreenWrapEntity.extend({maxVel:{x:500,y:500},type:ig.Entity.TYPE.B,pointValue:0,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0,1,2],true);this.randomizeAnimation();this.randomizeVelocity();},reset:function(x,y,settings){this.parent(x,y,settings);this.randomizeAnimation();this.randomizeVelocity();},randomizeAnimation:function(){this.currentAnim.gotoRandomFrame();this.currentAnim.timer.pause();this.currentAnim.angle=Math.random()*Math.PI*2;},randomizeVelocity:function(){velAngle=Math.random()*Math.PI*2;velMag=Math.random()*50+75;this.vel={x:Math.cos(velAngle)*velMag,y:Math.sin(velAngle)*velMag};},update:function(){this.screenWrap();this.parent();},breakAsteroid:function(score){if(score)ig.game.score+=this.pointValue;this.kill();}});AsteroidBig=Asteroid.extend({size:{x:90,y:90},offset:{x:5,y:5},animSheet:new ig.AnimationSheet('media/asteroid_big.png',100,100),pointValue:10,breakAsteroid:function(score){ig.game.spawnEntity(AsteroidSmall,this.pos.x+Math.random()*50,this.pos.y+Math.random()*50);ig.game.spawnEntity(AsteroidSmall,this.pos.x+Math.random()*50,this.pos.y+Math.random()*50);this.parent(score);}});AsteroidSmall=Asteroid.extend({size:{x:40,y:40},offset:{x:5,y:5},animSheet:new ig.AnimationSheet('media/asteroid_small.png',50,50),pointValue:20});AsteroidsGame=ig.Game.extend({started:false,gameOver:false,font:new ig.Font('media/04b03.font.png'),spaceBackground:new ig.Image('media/space.png'),titleScreen:new ig.Image('media/title.png'),gameOverScreen:new ig.Image('media/gameover.png'),score:0,ship:null,shipLife:new ig.Image('media/life.png'),asteroidTimer:new ig.Timer(25),asteroidTiming:15,asteroidMaxTiming:15,asteroidMinTiming:8,maxAsteroids:15,enemyTimer:new ig.Timer(15),init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.SPACE,'shoot');ig.EntityPool.enableFor(Bullet);ig.EntityPool.enableFor(Asteroid);},update:function(){this.parent();if(!this.started||this.gameOver){if(ig.input.state('shoot')){this.startGame();}}
else{this.considerSpawningAsteroid();this.considerSpawningEnemyShip();}},draw:function(){this.spaceBackground.draw(0,0);if(!this.started)this.titleScreen.draw(0,0);else if(this.started&&!this.gameOver){this.font.draw('LIVES:        SCORE: '+this.score,ig.system.width/2,15,ig.Font.ALIGN.CENTER);for(j=0;j<this.ship.health;j++){this.shipLife.draw(335+j*20,20);}
for(i=0;i<this.entities.length;i++){this.entities[i].draw();}}
else{this.gameOverScreen.draw(0,0);this.font.draw('LIVES:        SCORE: '+this.score,ig.system.width/2,15,ig.Font.ALIGN.CENTER);}},startGame:function(){this.ship=this.spawnEntity(Ship,ig.system.width/2,ig.system.height/2);this.started=true;this.gameOver=false;this.score=0;this.asteroidTiming=this.asteroidMaxTiming;this.enemyTimer.reset();this.spawnAsteroid();},endGame:function(){for(i=0;i<this.entities.length;i++){this.entities[i].kill();}
this.gameOver=true;},considerSpawningAsteroid:function(){if(this.asteroidTimer.delta()>=0&&this.getEntitiesByType(Asteroid).length<this.maxAsteroids){this.spawnAsteroid();}},spawnAsteroid:function(){if(Math.random()<0.5){if(Math.random()<0.5)
this.spawnEntity(AsteroidBig,Math.random()*ig.system.width,-100);else
this.spawnEntity(AsteroidBig,Math.random()*ig.system.width,ig.system.height+5);}
else{if(Math.random()<0.5)
this.spawnEntity(AsteroidBig,-100,Math.random()*ig.system.height);else
this.spawnEntity(AsteroidBig,ig.system.width+5,Math.random()*ig.system.height);}
if(this.asteroidTiming>this.asteroidMinTiming)this.asteroidTiming--;this.asteroidTimer.set(this.asteroidTiming);},considerSpawningEnemyShip:function(){if(this.score>50&&this.getEntitiesByType(EnemyShip).length==0&&this.enemyTimer.delta()>=0){this.spawnEnemyShip();}},spawnEnemyShip:function(){if(Math.random()<0.5){if(Math.random()<0.5)
this.spawnEntity(EnemyShip,-50,-50);else
this.spawnEntity(EnemyShip,-50,ig.system.height);}
else{if(Math.random()<0.5)
this.spawnEntity(EnemyShip,ig.system.width,-50);else
this.spawnEntity(EnemyShip,ig.system.width,ig.system.height);}
this.enemyTimer.reset();}});ig.main('#canvas',AsteroidsGame,60,800,800,1);});